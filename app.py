...
if st.button("✨ Hasilkan Analisis Gemini AI"):
    if not AI_READY:
        ...
        with st.spinner(f"Gemini AI sedang menyusun sintesis pembangunan wilayah {provinsi_terpilih}..."):
            try:
                # --- AUTO DETECT MODEL AI PALING UPDATE (Anti-404 Error) ---
                avail_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
                model_name = next((m for m in avail_models if 'flash' in m), avail_models[0] if avail_models else 'models/gemini-pro')
